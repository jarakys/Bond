...
class StarredAdapter(
    ...
    val chatlist: MutableList<ChatBrowsingListItem>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            TYPE_MESSAGE_OUTGOING -> {
                view = layoutInflater!!.inflate(R.layout.item_messages_outgoing_starred, parent, false)
    ...
    override fun getItemViewType(position: Int): Int {
        if (chatlist[position].getType() == TYPE_DATE){
            return TYPE_DATE
        } else if ((chatlist[position] as MessageItem).message.type == MessageType.System) {
            return TYPE_SYSTEM
        } else if ((chatlist[position] as MessageItem).message.type == MessageType.SystemMissedAudioCall || (chatlist[position] as MessageItem).message.type == MessageType.SystemMissedVideoCall) {
            return TYPE_SYSTEM_MISSED_CALL
        } else if ((chatlist[position] as MessageItem).message.type == MessageType.SystemAutoDelete) {
            return TYPE_SYSTEM_AUTO_DELETE
        } else if ((chatlist[position] as MessageItem).message.type == MessageType.AlertScreenshot) {
            return TYPE_SYSTEM_SCREENSHOT
        } else if ((chatlist[position] as MessageItem).message.type == MessageType.AlertScreenRecording) {
            return TYPE_SYSTEM_SCREENRECORDING
        } else if ((chatlist[position] as MessageItem).message.type == MessageType.SystemTemporaryChat) {
            return TYPE_TEMPORARY_DELETE
        }
        else{

            return when((chatlist[position] as MessageItem).message.isOutgoing){
                true -> {
                    when((chatlist[position] as MessageItem).message.type) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is StarredAdapter.GeneralMessageViewHolder) {
            holder.bind(position)
        }
        else if (holder is StarredAdapter.DateSecViewHolder) {
            holder.bind(position)
        }
        else if (holder is StarredAdapter.DeletedMessageViewHolder) {
            holder.bind(position)
        } else if(holder is StarredAdapter.SystemMessagesSecViewHolder){
            holder.bind(position)
        } else if (holder is StarredAdapter.SystemAutoDeleteMessageSecViewHolder) {
            holder.bind(position)
        } else if (holder is StarredAdapter.SystemScreenshotMessageSecViewHolder) {
            holder.bind(position)
        } else if (holder is StarredAdapter.SystemScreenRecordingMessageSecViewHolder) {
            holder.bind(position)
        } else if (holder is StarredAdapter.SystemMissedCallMessagesSecViewHolder) {
            holder.bind(position)
        } else if (holder is StarredAdapter.SystemAlertCopyMessageSecViewHolder) {
            holder.bind(position)
        } else if (holder is StarredAdapter.SystemTemporaryDeleteMessageSecViewHolder) {
            holder.bind(position)
        }
    ...
    open inner class GeneralMessageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        open fun bind(position: Int) {
            ...
            if (chatlist[position] is MessageItem){
                messageItem = chatlist[position] as MessageItem
                ...
                if (position < chatlist.size-1) {
                    ...
                    if (chatlist[position + 1].getType() != TYPE_DATE && (chatlist[position + 1] as MessageItem).message.sender == messageItem.message.sender == true && (chatlist[position + 1] as MessageItem).message.type.isSystemMessage != true) {
                ...
                if (chatBrowsingViewModel.getSelectedMessages().any { it.message.ID == (chatlist[position] as MessageItem).message.ID }) {
...
//            Log.i("count","nonEmojiCount = ${nonEmojiCount}")
            ...
            if (messageItem.message.repliedToMsgId != "0"){
                ...
                var originalMsg = chatlist.filterIsInstance<MessageItem>().firstOrNull() { it.message.ID == messageItem.message.repliedToMsgId }?.message
...
//                    NavHostFragment.findNavController(chatBrowsingFragment).navigate(action, extras)
    ...
    private fun handleClickOnView(position: Int,itemView: View) {
        chatBrowsingViewModel.getSelectedMessages().forEach {
            if (chatlist[position] is MessageItem){
                if(it.message.ID == (chatlist[position] as MessageItem).message.ID) {
                    chatBrowsingViewModel.removeItemSelectedMessages(chatlist[position] as MessageItem)
                    itemView.setBackgroundResource(R.drawable.flag_transparent)
                    checkStarAction()
    ...
    private fun updateDowloadStatus(message: Message, fileType: Int, progressPercentagTV: TextView? , loadingProgressbar: ProgressBar?,counter: Int,handleAction: () -> Unit) = GlobalScope.launch {
        ...
        Log.i("progress ",progress.toString() + "messageId = "+ message.ID)
        when {
            progress == 100 -> {
                delay(250)
...
//            }
    ...
    open inner class DocumentMessageViewHolder(itemView: View): GeneralMessageViewHolder(itemView) {
        ...
        override fun bind(position: Int) {
            ...
            chatBrowsingViewModel.drawDocumentUIS(messageItem.message.type,context,fileType_IV,fileType_TV,fileType)
...
//            playingHolder!!.audioSeekBar.isEnabled = true
    ...
    open inner class DeletedMessageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        open fun bind(position: Int) {
            if (chatlist[position] is MessageItem) {
                var messageItem = chatlist[position] as MessageItem
                if (position < chatlist.size-1) {
                    if (group_layout != null) {
                        group_layout.visibility = View.GONE
                    }
                    if (chatlist[position + 1].getType() != TYPE_DATE && (chatlist[position + 1] as MessageItem).message.sender == messageItem.message.sender) {
                        message_arrow.visibility = View.INVISIBLE
                        if (messageItem.message.isOutgoing)
                            messageFrame_relativeLayout.setBackgroundResource(R.drawable.bg_msg_right_fullrounded)
                        else {
    ...
    open inner class DateSecViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        open fun bind(position: Int) {
            val date =  (chatlist[position] as DateItem).date
    ...
    open inner class SystemAutoDeleteMessageSecViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        open fun bind(position: Int) {
            var text = (chatlist[position] as MessageItem).message.body
    ...
    open inner class SystemTemporaryDeleteMessageSecViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        open fun bind(position: Int) {
            itemView.autoDelete_section_txt.text = (chatlist[position] as MessageItem).message.body
    ...
    open inner class SystemAlertCopyMessageSecViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int) {
            var messageItem: MessageItem = chatlist[position] as MessageItem
            outInMessage_replyPart_ConstraintLayout.setOnClickListener {
            ...
            if (position < chatlist.size-1) {
                ...
                if (chatlist[position + 1].getType() != TYPE_DATE && (chatlist[position + 1] as? MessageItem)?.message?.isOutgoing == messageItem.message.isOutgoing) {
                    message_arrow.visibility = View.INVISIBLE
    ...
    fun addInSelectedList(position: Int){
        chatBrowsingViewModel.addToSelectedMessages(chatlist[position] as MessageItem)
        chatBrowsingViewModel.setLongPressedTitle(chatBrowsingViewModel.getSizeSelectedMessages().toString())
    }
    ...
    override fun searchWithText(word: String, firstPosition: Int, lastPosition: Int) {
        ...
        for (i in firstPosition..lastPosition) {
            val message = (chatlist[i] as? MessageItem)?.message
            if (message != null) {
    ...
    override fun searchUpArrow(textView: TextView, firstPosition: Int, lastPosition: Int, recyclerView: RecyclerView) {
        ...
        if (searchWord.trim().isNotEmpty()) {
            ...
            for (i in firstPosition..chatlist.size - 1) {
                val message = (chatlist[i] as? MessageItem)?.message
                if (message != null) {
...
//            notifyDataSetChanged()
    ...
    fun searchForSpecificMessage(msgId: String) {
        var message = chatlist.filterIsInstance<MessageItem>().firstOrNull {
            it.message.ID == msgId
        }
        message?.let {
            val messagePosition = chatlist.indexOf(it)
