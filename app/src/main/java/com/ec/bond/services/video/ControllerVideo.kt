...
object ControllerVideo {
    ...
    fun initCamera(
        ...
        val lifecycleOwner = WeakReference(context as LifecycleOwner)
        destroyCamera()
    ...
    private fun startCamera(
        ...
        try {
            ...
            displayManager =
                (lifecycleOwner.get() as Context).getSystemService(Context.DISPLAY_SERVICE) as DisplayManager
            displayManager.registerDisplayListener(rotationListener, null)

            ControllerVideo.displayId = displayId

            rotationCurrent = displayManager.getDisplay(displayId).rotation

            isLandscape =
                (lifecycleOwner.get() as Context).resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE
    ...
    fun destroyCamera() {
        if (this::cameraProvider.isInitialized) {
            try {
                ...
                Log.e("ControllerVideo", "[destroyCamera] error: $e")
            }
        }
        if (this::displayManager.isInitialized) displayManager.unregisterDisplayListener(
            rotationListener
        )
        released = true
    }
