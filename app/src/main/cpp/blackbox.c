...
int FlagDebug=0;
char bbhostname[128]={"51.255.236.70"};
int bbport=443;
char bbtoken[256]={""};
...
//                    bb_watermark_jpeg(localfilenamemsg);
           ...
           *sp=0;
           ...
           j=strlen(njr);
           njr[j-1]=0;
           sprintf(jsonadd,",\"originfilename\":\"%s\",\"localfilename\":\"%s\"}",originfilename,localfilenamemsg);
   ...
   while(1){
       ...
       if(strcmp(msgtype,"file")==0){
           ...
           *sp=0;
           ...
           sprintf(jsonadd,",\"originfilename\":\"%s\",\"localfilename\":\"%s/Documents/%s\",\"autodownload\":\"%s\"}",originfilename,getenv("HOME"),localfilenamemsg,autodownload);
           strncat(njr,jsonadd,1024);
           len_njr=strlen(njr);
           if(verbose) printf("njr: %s\n",njr);
           if(bb_check_autodownload(originfilename,autodownloadphotos,autodownloadvideos,autodownloadaudios,autodownloaddocuments)==1){
   ...
   while(1){
       ...
       if(strcmp(msgtype,"file")==0){
           ...
           *sp=0;
           ...
           j=strlen(njr);
           njr[j-1]=0;
           sprintf(jsonadd,",\"originfilename\":\"%s\",\"localfilename\":\"%s/Documents/%s\"}",originfilename,getenv("HOME"),localfilenamemsg);
           strncat(njr,jsonadd,1024);
           len_njr=strlen(njr);
           if(verbose) printf("njr: %s\n",njr);
           //**************************************
...
//             bb_watermark_jpeg(localfilenamemsg);
//        }
        if(verbose) printf("originfilename: %s localfilenamemsg: %s\n",originfilename,localfilenamemsg);
       }
       //REPLACE MSGBODY
       newtmsg=malloc(strlen(tmsg)+64);
       strcpy(newtmsg,tmsg);
...
/**
* FUNCTION TO REFRESH THE CONTACTS AND SEARCH/GET CONTACT, THE RESULT MUST BE FREE()
*/
char * bb_get_contacts_refresh(char *search,int contactid,int flagsearch,int limitsearch,char *pwdconf){
    char *msg=NULL;
    char *crt=NULL;
    char bodymsg[512];
    char *bodymsgenc=NULL;
    ...
    // SET SERVER ACCOUNT
    strcpy(recipient,"0000001");
    sprintf(bodymsg,"{\"search\":\"%s\",\"contactid\":\"%d\",\"flagsearch\":\"%d\",\"limitsearch\":\"%d\",\"refresh\":\"Y\"}",search,contactid,flagsearch,limitsearch);
    //DECRYPT CONFIGURATION
    replyconf=bb_load_configuration(pwdconf,conf);
    bb_json_getvalue("answer",replyconf,answer,63);
    free(replyconf);
...
//   if(!bb_encrypt_file(originfilename,filenameenc,filepwd)){
   ...
   {
         sprintf(serverfilenamepath,"%s/Documents/%s",getenv("HOME"),serverfilename);
         rename(cachefilename,serverfilenamepath);
         sprintf(serverfilenamepathenc,"%s/Documents/%s.enc",getenv("HOME"),serverfilename);
...
//      if((originfilename[i-1]=='g' && originfilename[i-2]=='p' && originfilename[i-3]=='j' && originfilename[i-4]=='.') ||
//      (originfilename[i-1]=='g' && originfilename[i-2]=='e' && originfilename[i-3]=='p' && originfilename[i-4]=='j'  && originfilename[i-5]=='.')){
//         bb_watermark_jpeg(localfilename);
//       }
   }
...
char * bb_get_photo(char *uniquefilename,char *pwdconf){
   ...
   hashlen=0;
   cx=0;
   replysend=malloc(4096);
   sprintf(replysend,"{\"answer\":\"OK\",\"message\":\"File download completed.\",\"filename\":\"%s\",\"localfilename\":\"%s\"}",filenameenc,localfilename);
   return(replysend);
...
sprintf(filename,"%s/Documents/bbconfiguration.enc",getenv("HOME"));    
fp=fopen(filename,"w");
fprintf(fp,"%s",encb64);
fclose(fp);
...
char * bb_tls_sendmsg(char *hostname,int port,char *msg,int *lenreply){
    ...
    char publickey[]={"MIGbMBAGByqGSM49AgEGBSuBBAAjA4GGAAQBWSPEtlcBsDmeXmeubcDzvbtZZMSm+TDcH1o+aUZFy3B77DSY1zXPTzjCFT+zGxVspW/TBnzAxob8uYZ1RD6SuuUB/CA8S/kRq2DDyt7Nv1/UdvhqNa26ZauEbCYQJ+FALDgMDeyR3kh27s6UZUBSmSO3melTmRKB4ztKJBna9HTbUWM="};
...
"BgNVBAMMC1hYWFhYWFhYIENBMB4XDTIxMDYxNTA5MzY0OVoXDTMxMDYxNTA5MzY0\n"
"OVowVDELMAkGA1UEBhMCWFgxETAPBgNVBAoMCFhYWFhYWFhYMRgwFgYDVQQLDA9Y\n"
"WFhYWFhYWCBUTFMgQ0ExGDAWBgNVBAMMD1hYWFhYWFhYIFRMUyBDQTCBmzAQBgcq\n"
"hkjOPQIBBgUrgQQAIwOBhgAEAL0QK1pyP4fcX1Bp2FsNhCgI5WDOIETAS/VJszqN\n"
"uePwdCCO5/ylkq00okVBGcjli9sQzEQmRdNxCN/8HmK8JacbAfQ6H9wNHw+QGMFo\n"
"j5+eCyf8rBnIUKDGq3Pu7KSUy/qYpWAgwDWV7+v5ZcMOGUADEAEloMPLYAMPc4oi\n"
"orVI5mrvo4IBBDCCAQAwDgYDVR0PAQH/BAQDAgEGMBIGA1UdEwEB/wQIMAYBAf8C\n"
"AQAwHQYDVR0OBBYEFFSVKivzeju8FrqLMr9VyCvsOgMZMB8GA1UdIwQYMBaAFLdh\n"
"IXFKVWFGcRhluC7ih2LnMGXCMD8GCCsGAQUFBwEBBDMwMTAvBggrBgEFBQcwAoYj\n"
...
"hwJCAaTt7x1g4dAySsun0HcekhqF7RH5U01ruKcAWrQjCpWQzgU3ARfr8Wi5rMjk\n"
"tO3gM4y5BFV3Uw8SxLFJGtmUig58AkEfUQWEm0RYEhCZBLwzXHcxzXCCu7G4dvFN\n"
"uOcRg8fmMDIPAJopkBnLqA/VlmXfpB1zNmglbMA01xE380xanVoWbA==\n"
"-----END CERTIFICATE-----\n"
"-----BEGIN CERTIFICATE-----\n"
"MIIDEzCCAnWgAwIBAgIBAjAKBggqhkjOPQQDBDBOMQswCQYDVQQGEwJYWDERMA8G\n"
"A1UECgwIWFhYWFhYWFgxETAPBgNVBAsMCFhYWFhYWFhYMRkwFwYDVQQDDBBYWFhY\n"
"WFhYWCBSb290IENBMB4XDTIxMDYxNTA5MzQ1N1oXDTMwMTIzMTIzNTk1OVowVDEL\n"
...
"BgUrgQQAIwOBhgAEACnRg2AZu0eIOSAyzXGuCxkyk9BE4eNmLxUwyHy0OZFGiZX4\n"
"t2BsSK8LJUkG/NdlaGc/SMAQfiQOwaaJ02EWphciAW3cQkgAr/1bvgFU3KZ4ijTH\n"
"bQWJM/2iXTMkwmcPrxpXVDMe9ZZ247e9aaVsxHAVGNwwGQjrGb8LOifd6sB6+7kQ\n"
"o4H6MIH3MA4GA1UdDwEB/wQEAwIBBjAPBgNVHRMBAf8EBTADAQH/MB0GA1UdDgQW\n"
"BBS3YSFxSlVhRnEYZbgu4odi5zBlwjAfBgNVHSMEGDAWgBSFhxyZjiDgdHTSajCx\n"
"8f3bRxZZpTA8BggrBgEFBQcBAQQwMC4wLAYIKwYBBQUHMAKGIGh0dHBzOi8va3J5\n"
...
"MAsGCSsGAQQBAAEHCTAKBggqhkjOPQQDBAOBiwAwgYcCQgF/yj+jFWW1Aid2/JRD\n"
"0SB0JyixmwWyRpEhIpZuzNlPUCrZCR8gcR3OWGjva3TtubTwgUYVctmG8Oa/YVfz\n"
"xdoMBQJBS8Tr6t/snoEYwPEMyKMlrri3G0PcO6M/ByH9uFBk93KwIBnjouj2NhzT\n"
"3HbbZX68t5e7iwDUIi/zJT0HDFXEVAE=\n"
"-----END CERTIFICATE-----\n"
"-----BEGIN CERTIFICATE-----\n"
"MIICdTCCAdegAwIBAgIBATAKBggqhkjOPQQDBDBOMQswCQYDVQQGEwJYWDERMA8G\n"
"A1UECgwIWFhYWFhYWFgxETAPBgNVBAsMCFhYWFhYWFhYMRkwFwYDVQQDDBBYWFhY\n"
"WFhYWCBSb290IENBMB4XDTIxMDYxNTA5MzQyMVoXDTMwMTIzMTIzNTk1OVowTjEL\n"
...
"IwOBhgAEAQYhkzrbH7DKH0FcNGYh8ATNCBkV7URCa7kfYX+2/h+dojISMb7Hbrun\n"
"+OloVvABwkTY7NwGyml6rx5QWdVGerwvAMlciYrwKDdUZcu8PpP0pkuADBIlm6Jl\n"
"ON+GdDI++7uKeV6KRQxbAa++chlQJMDnmGBqyjxmH9P2aFOlxrpIzwZyo2MwYTAO\n"
"BgNVHQ8BAf8EBAMCAQYwDwYDVR0TAQH/BAUwAwEB/zAdBgNVHQ4EFgQUhYccmY4g\n"
"4HR00mowsfH920cWWaUwHwYDVR0jBBgwFoAUhYccmY4g4HR00mowsfH920cWWaUw\n"
"CgYIKoZIzj0EAwQDgYsAMIGHAkIArWcFKr0fKHK95Rrj1KTtjrfFAucGJ3hm66hl\n"
"dOzn+mpK5mSe9qSI0n2ItgldygShr+HycICZxDHoa72RlYorcBUCQREYppx5SvVv\n"
"c9hErr09hjV/1pJLCcH5d9buhU4/eteqwDUxjLeIXRpKbRUcbJ4egaa3P+N3ZSiM\n"
"jutqMSj577Ai\n"
"-----END CERTIFICATE-----\n",8192);
    ...
    if ( SSL_connect(ssl) == -1 ){   /* perform the connection */
        ...
        LOGE("2004 - error connecting TLS to server %s", server);
        close(server);
        goto CLEANUP;
    }
    //CERTIFICATE PINNING
    ...
    if(bytes<=0){
        strcpy(error,"2013 - Error reading  message ");
        close(server);
        goto CLEANUP;
    }
    reply[bytes]=0;
...
char * bb_tls_sendfile(char *hostname,int port,char *msg,int *lenreply,char *filename){
    ...
    */
    ...
    if ( SSL_connect(ssl) == -1 ){   /* perform the connection */
        sprintf(error,"2004 - error connecting TLS to server [%s]",hostname);
        LOGE("2004 - error connecting TLS to server %s", hostname);
        close(server);
        goto CLEANUP;
    }
    //CERTIFICATE PINNING
...
char * bb_tls_getencryptedfile(char *hostname,int port,char *msg,char *filename){
    //*** PUBLIC KEY OF SERVER CERTIFICATE TO PIN IN DER FORMAT ENCODED IN BASE64 (CHANGE IT FOR PRODUCTION)
   // char publickey[]={"MIGbMBAGByqGSM49AgEGBSuBBAAjA4GGAAQAl1lnGjoKNPOXNsAYjGoHGsQlMdOxFkLnLdLjeErL+cPLqIw2FHksrYjxNSrOhaOfghrs3/J9+J326jrRsMq/o5IApnbcjguRVQFvytmrmWcrg+tcHZITMrxJSYolqy2t3fUs57LGT1FBlflQ5JwA2hMMETkvn419EhwhqD3gOs1PB/E="};
    char publickey[]={"MIGbMBAGByqGSM49AgEGBSuBBAAjA4GGAAQBWSPEtlcBsDmeXmeubcDzvbtZZMSm+TDcH1o+aUZFy3B77DSY1zXPTzjCFT+zGxVspW/TBnzAxob8uYZ1RD6SuuUB/CA8S/kRq2DDyt7Nv1/UdvhqNa26ZauEbCYQJ+FALDgMDeyR3kh27s6UZUBSmSO3melTmRKB4ztKJBna9HTbUWM="};
    //******************************************************************************************************
    SSL_CTX *ctx;
    char error[256];
    ...
    sprintf(fnc,"%s/Documents/%s.enc",getenv("HOME"),filename);    
    sprintf(fncd,"%s/Documents/%s",getenv("HOME"),filename);        
    sprintf(fncdw,"%s/Documents/%s.download",getenv("HOME"),filename);            
...
char * bb_get_cert(char *sender,char *recipient,char *token,char *pwd){
    ...
    sprintf(fname,"%s/Documents/cache",getenv("HOME"));    
    //printf(fname);
    //exit(0);
    if (stat(fname, &sb) != 0)
    {
    ...
    sprintf(fname,"%s/Documents/cache/%s.enccrt",getenv("HOME"),recipient);
...
char * bb_load_configuration(char *pwdconf,char * conf){
     ...
     fp=fopen(filename,"r");
     if(fp==NULL){
         strcpy(error,"3800 - configuration file is missing: bbconfiguration.enc\n");
         goto CLEANUP;
     }    
...
int bb_video_send(unsigned char *videopacket,unsigned short int packetlen){
    ...
    //debug
if(FlagDebug<=2){
    bb_hexdump("bb_video_send feed",videopacket,packetlen);
    FlagDebug++;
}
...
*/
unsigned char * bb_video_receive(int *dplen){
      int x;
      time_t st,tt,ptr,am;
      ...
      while(1){
          ...
          LOGE("bb_swtp_read_data response %d", x);
           if(x==-3){
...
char * bb_copy_file_to_cache(char *originfilename){
    ...
    sprintf(destinationfilename,"%s/Documents",getenv("HOME"));
    if (stat(destinationfilename, &sb) != 0) mkdir(destinationfilename, S_IRWXU | S_IRWXG);
    sprintf(destinationfilename,"%s/Documents/%s",getenv("HOME"),buffer); 
    if(strlen(suffix)==4)
...
char * bb_cache_file_name(char * originfilename){
    ...
    if(originfilename[x-5]=='.') strncpy(suffix,&originfilename[x-5],5);
    if(originfilename[x-4]=='.') strncpy(suffix,&originfilename[x-4],4);
    sprintf(d,"%s/Documents/%s%s",getenv("HOME"),hash,suffix);
    memset(suffix,0x0,32);
    memset(h,0x0,128);
    memset(hash,0x0,256);
    return(d);
...
//*** ORIGIN: ../bb_encode_decode_base64.c
/*#include "blackbox.h"
...
//*** ORIGIN: ../bb_encrypt_decrypt_buffer.c
/*#include "blackbox.h"
...
//*** ORIGIN: ../bb_encrypt_decrypt_buffer_aes_gcm.c
/*#include "blackbox.h"
...
//*** ORIGIN: ../bb_encrypt_decrypt_buffer_camellia_ofb.c
/*#include "blackbox.h"
...
//*** ORIGIN: ../bb_encrypt_decrypt_buffer_chacha20.c
/*#include "blackbox.h"
...
char json[4096];
destination[0]=0;
memset(json,0x0,4096);
char msg1[256];
char msg2[256];
char msg3[256];
char *a;
sprintf(msg1,"{\"dt\":\"2019-12-10\",\"txt\":\"msg nr.1\"}");
sprintf(msg2,"{\"dt\":\"2019-12-11\",\"txt\":\"msg nr.2\"}");
sprintf(msg3,"{\"dt\":\"2019-12-12\",\"txt\":\"msg nr.3\"}");
sprintf(json,"{\"name\":\"Samuele\",\"address\":  \"Via dei \\\"buchi grassi\",\"msgspinco\":[%s,%s,%s]}",msg1,msg2,msg3);
printf("json: %s\n",json);*/
//FILE *f;
//f=fopen("test.json","r");
//fgets(json,4095,f);
//fclose(f);
//if(bb_json_getvalue("encryptedkeyntru",json,destination,4095)==0){
// printf("encryptedkeyntru not found");
//}
//printf("encryptedkeyntru:%s\n",destination);
/*if(bb_json_getvalue("address",json,destination,maxlen)==0)
    printf("Field not found");
else
    printf("Destination %s\n",destination);*/
/*
a=bb_json_getvalue_fromarray("msgspinco",json,0);
if(a!=NULL){
...
for(i=0;i<len;i++){
    ...
    if(json[i]=='"' && startv>0 && json[i-1]!='\\' && endv==0 & endn>0 && startn>0){
        ...
        if(strcmp(fieldname,name)==0){
            if(strlen(fieldvalue)>maxlen && maxlen>2) fieldvalue[maxlen-2]=0;
...
*/
//*** ORIGIN: ../bb_encrypt_decrypt_buffer_ec.c
/*#include "blackbox.h"
void main(void){
size_t secret_len;
...
//*** ORIGIN: ../bb_ecdhe_compute_secretkey.c
/*#include "blackbox.h"
...
*/
//*** ORIGIN: ../bb_encrypt_decrypt_file.c
/*#include "blackbox.h"
...
//*** ORIGIN: ../bb_encrypt_decrypt_file_aes_gcm.c
/*#include "blackbox.h"
...
//*** ORIGIN: ../bb_encrypt_decrypt_file_camellia_ofb.c
/*#include "blackbox.h"
...
//*** ORIGIN: ../bb_encrypt_decrypt_file_chacha20.c
...
if (i != 0)
LOGE("network feed  %s\n", buff);
LOGE("network feed  %04x ", i);
        }
LOGE("network feed %02x", pc[i]);
...
//*** ORIGIN: ../bb_encrypt_decrypt_buffer_aes_ofb.c
//*******************************************************************
...
int bb_svtp_send_data(SVTP *svtp,unsigned char *datapacketorigin,unsigned short int dplen){
    ...
    if(bytesent==-1){
        ...
        LOGE("SVTP - Error sending data packet - %d", errno);
        return(0);
    }
    //UPDATE CONTEXT
    svtp->sq++;
...
int bb_swtp_read_data(SWTP *swtp,unsigned char *datapacket){
    ...
    while(1) {
        ...
        if(bytesrcv<=0 && tt-st<10){
            //printf("sleeping 1ms \n");
            //LOGE("58509 - Error receiving data packet - %d", errno);
            usleep(1000);
            continue;
        }
        break;
...
int bb_swtp_send_data(SWTP *swtp,unsigned char *datapacket){
    ...
    if(bytesent==-1){
        ...
        LOGE("SWTP - Error sending data packet - %d", errno);
        return(0);
    }
    //printf("Byte sent: %u\n",bytesent);
...
/*
    ...
    timeout.tv_sec = 1;
    timeout.tv_usec = 0;
    setsockopt(swtp->fdsocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,sizeof(timeout));
*/
    ...
    if(strcmp(ipdestination,"0.0.0.0")==0){
      ...
      if(!inet_pton(AF_INET, ipdestination, &(swtp->destination.sin_addr))){
          ...
          LOGE("55853 - invalid ip adress for udp socket");
          return(0);
      }
    }
    StatusVideoCall=0;
...
strcpy(serveripaddress,"51.255.236.70");
...
void bb_push_messages_client_android(char *mobile_number, JNIEnv *env, jobject object, jmethodID methodId) {
    int port=4444;
    char serveripaddress[256];
    unsigned char iv[128];
    unsigned char signature[128];
...
//CONNECTING TO TCP SERVER
    ...
    sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd == -1) {
        LOGE("26202 - Socket creation failed...\n");
        fprintf(stderr,"26202 - Socket creation failed...\n");
        fprintf(stderr,"26203 - Waiting 15 seconds to try again \n");
        sleep(15);
        close(sockfd);
    ...
    servaddr.sin_port = htons(port);
    if (connect(sockfd, (struct sockaddr*)&servaddr, sizeof(servaddr)) != 0) {
        LOGE("26203 - connection with the server failed...");
        fprintf(stderr,"26203 - connection with the server %s failed...\n",serveripaddress);
        fprintf(stderr,"26203 - Waiting 15 seconds to try again \n");
        sleep(15);
        close(sockfd);
...
//READING PUSH MESSAGES
    ...
    while(1){
        ...
        if(r<0){
            if(verbose) printf("26205 - Error reading socket,reconnecting...[%d]\n",r);
            if(PushServerFd==0) return;
            //LOGE("Push Server Socket error: %d\n", errno);
            if (errno==11) {
                continue;
            } else if(errno==35){
                usleep(100000);
                ...
                sleep(1);
                LOGE("26205 - Error reading socket,reconnecting...");
                goto START;
            }
        }
        if(verbose) bb_hexdump("26211 -Received from Push Server",bufr,r);
        ...
        if(strcmp(cleardata,"newmsg")==0){
            if(verbose) printf("26006 - Call back for new msg\n");
            (*env)->CallVoidMethod(env, object, methodId, 0);
        }
        if(strcmp(cleardata,"voicecall")==0){
            if(verbose) printf("26007 - Call back for new voice call\n");
            (*env)->CallVoidMethod(env, object, methodId, 1);
        }
        if(strcmp(cleardata,"videocall")==0){
            if(verbose) printf("26007 - Call back for new voice call\n");
            (*env)->CallVoidMethod(env, object, methodId, 2);
        }
    }
    LOGE("Close push client...");
    close(sockfd);
}
...
//*** ORIGIN: ../pushmessages/bb_client_connect_pushserver.c
/*#include "blackbox.h"
...
//unlink("/tmp/1f5a97350efac13d54068432fff1f31d5df719aa6038075881e549c1d4422af1");
printf("************************************\n");
memset(pwdconf,0x0,8192);
...
void bb_keyget_userpwd(char *keyuser,unsigned char * pwdconfenc,int pwdconfenclen,char *tmpfolder){
    ...
    char hardcoded[256]={"2999be915ed1a8f6232982347b0cc18b050abee06b687f295c9898651dc87126"};
    unsigned char hash[128];
    ...
    strcat(tmpfile,"/1f5a97350efac13d54068432fff1f31d5df719aa6038075881e549c1d4422af1");
    f=fopen(tmpfile,"rb");
    free(tmpfile);
    if(f==NULL) return;
...
void bb_clean_tmp_masterpwdfile(char *tmpfolder){
    ...
    strncat(tmpfile,"/1f5a97350efac13d54068432fff1f31d5df719aa6038075881e549c1d4422af1",1023); 
    bb_crypto_random_data(rd);
    f=fopen(tmpfile,"wb");
    if(f==NULL) return;
    for(i=0;i<=9;i++)
...
//    return; // CANCELLATION SUSPENDED BECAUSE OF SPEED COMPLAINTS (temporary change)
    if(strlen(DocumentPath)==0){
        ...
        strncat(DocumentPath,"/Documents/",32);
    }
