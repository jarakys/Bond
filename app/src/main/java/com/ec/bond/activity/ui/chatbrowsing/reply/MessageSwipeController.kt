...
class MessageSwipeController(private val context: Context,private val viewModel: ChatBrowsingViewModel, private val swipeControllerActions: SwipeControllerActions) :
    ...
    override fun onChildDraw(
        ...
        if (viewModel.chatTypeRef.messages[viewHolder.adapterPosition] !is DateItem) {
            if ((viewModel.chatTypeRef.messages[viewHolder.adapterPosition] as? MessageItem)?.message?.type?.isDocumentMessage == true || (viewModel.chatTypeRef.messages[viewHolder.adapterPosition] as? MessageItem)?.message?.type == MessageType.Text || (viewModel.chatTypeRef.messages[viewHolder.adapterPosition] as? MessageItem)?.message?.type == MessageType.Photo || (viewModel.chatTypeRef.messages[viewHolder.adapterPosition] as? MessageItem)?.message?.type == MessageType.Video || (viewModel.chatTypeRef.messages[viewHolder.adapterPosition] as? MessageItem)?.message?.type == MessageType.Audio) {
    ...
    private fun drawReplyButton(canvas: Canvas) {
        ...
        shareRound.colorFilter =
                PorterDuffColorFilter(ContextCompat.getColor(context, R.color.reply_color), PorterDuff.Mode.MULTIPLY)
